...
-- Guide table for 2026
DROP TABLE IF EXISTS public.guide_2026 CASCADE;
CREATE TABLE public.guide_2026 (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    description text NULL,
    years text NULL,
...
-- GIN index for trigram text similarity search on description
CREATE INDEX guide_2026_description_trgm_idx ON public.guide_2026 USING gin (description gin_trgm_ops);
CREATE INDEX guide_2026_years_idx ON public.guide_2026 (years);

-- ============================================================
-- VIN SEARCH FUNCTION WITH GUIDE MATCHING
-- ============================================================

-- Search VINs and find matching guide descriptions
...
BEGIN
    ...
    WITH vin_matches AS (
        ...
        -- VINs in table are 9-11 chars, '?' represents wildcards
        -- Extra characters in search VIN are treated as wildcards
        SELECT 
            v.id,
            v.vin,
            v.model_year,
        ...
        WHERE 
            -- Match by converting both VIN patterns to SQL LIKE patterns
            -- Replace '?' with '_' for single character wildcard in stored VIN
            -- Truncate search VIN to length of stored VIN for comparison
            CASE 
                -- If search VIN is longer or equal to stored VIN length, 
                -- compare up to stored VIN length with wildcard substitution
                WHEN v_vin_length >= length(v.vin) THEN
                    substring(p_vin, 1, length(v.vin)) LIKE replace(v.vin, '?', '_')
                -- If search VIN is shorter, pad with wildcards
                ELSE
                    p_vin || repeat('_', length(v.vin) - v_vin_length) LIKE replace(v.vin, '?', '_')
            END
        LIMIT 100 -- Safety limit on VIN matches
    ),
    guide_matches AS (
        -- For each VIN match, find top 5 most similar guide descriptions
        SELECT 
            vm.id as vin_id,
            vm.vin,
            vm.model_year,
            ...
            jsonb_agg(
                jsonb_build_object(
                    ...
                    'years', g.years,
                    'similarity_score', similarity(COALESCE(vm.description, ''), COALESCE(g.description, ''))
                )
                ORDER BY similarity(COALESCE(vm.description, ''), COALESCE(g.description, '')) DESC
            ) FILTER (WHERE g.id IS NOT NULL) as guide_results
        FROM vin_matches vm
        LEFT JOIN LATERAL (
            SELECT 
                g.id,
                g.description,
                g.years,
                similarity(COALESCE(vm.description, ''), COALESCE(g.description, '')) as sim_score
            FROM public.guide_2026 g
            WHERE 
                g.description IS NOT NULL 
