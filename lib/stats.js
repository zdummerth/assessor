...
export function computeGroupedNumeric(
  ...
  for (const { rows: bucketRows, tuple } of buckets.values()) {
    ...
    // Trim (if requested) and sort ascending
    const trimmedSorted = tf
      ? trimIQR(rawVals, tf).sort((a, b) => a - b)
      : [...rawVals].sort((a, b) => a - b);

    const s = trimmedSorted;
    const n = s.length;
    ...
    // ⬇️ include the trimmed, sorted values for downstream histograms
    out.push({
      group_values,
      median,
      avg,
      ...
      max,
      count: n,
      values: s, // NEW: trimmed & sorted numeric array
    });
  }
...
export function summarizeStructureFlat(structure = {}, sections = []) {
  const KNOWN_TYPES = ["floor", "attic", "basement", "crawl space", "addition"];

  // Normalize a type to a stable key suffix (spaces -> underscores)
  const typeKey = (t) => t.toLowerCase();

  // Initialize all known type totals to 0 (so missing types still appear)
  const perType = {};
  for (const t of KNOWN_TYPES) {
    const k = typeKey(t).replace(/\s+/g, "_"); // e.g. 'crawl space' -> 'crawl_space'
    perType[`finished_area_${k}`] = 0;
    perType[`unfinished_area_${k}`] = 0;
    perType[`sections_count_${k}`] = 0;
  }

  let totalFinished = 0;
  let totalUnfinished = 0;

  const floorsOverall = new Set(); // all distinct floor_number values
  const floorsAbove = new Set(); // floor_number > 0 only

  for (const sec of sections || []) {
    const rawType = (sec?.type ?? "").toString().toLowerCase();
    const k = rawType.replace(/\s+/g, "_");
    const finished = Number(sec?.finished_area) || 0;
    const unfinished = Number(sec?.unfinished_area) || 0;

    // Tally per-type only if it's one of the known types
    if (KNOWN_TYPES.includes(rawType)) {
      perType[`finished_area_${k}`] += finished;
      perType[`unfinished_area_${k}`] += unfinished;
      perType[`sections_count_${k}`] += 1;
    }

    // Always included in grand totals
    totalFinished += finished;
    totalUnfinished += unfinished;

    // Story counting (distinct floor_number values)
    const fn = Number(sec?.floor_number);
    if (Number.isFinite(fn)) {
      floorsOverall.add(fn);
      if (fn > 0) floorsAbove.add(fn);
    }
  }
  ...
  return {
    ...
    // grand totals
    total_finished_area: totalFinished,
    total_unfinished_area: totalUnfinished,

    // story counts
    total_stories_above_grade: floorsAbove.size,
    total_stories_overall: floorsOverall.size,

    // per-type flat totals (zeros included for missing types)
    ...perType,
  };
}

/** Convenience for array-of-items: [{ structure, sections }, ...] */
export function summarizeStructuresArrayFlat(items = []) {
  return items.map(({ structure, sections }) =>
    summarizeStructureFlat(structure, sections)
  );
}
